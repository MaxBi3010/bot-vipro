...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    try {
        const res = (await axios.get(`https://api.joshweb.click/api/xsearch?q=${encodeURIComponent(search)}`)).data;
...
module.exports.handleReply = async function ({ event, api, Currencies, Users, handleReply }) {
    ...
    try {
        const res = await axios.get(`https://api.joshweb.click/api/xdl?q=${encodeURIComponent(chosenVideo.link)}`);
        ...
        if (contentLength > 10000 * 10000 * 10000) { // 24MB
