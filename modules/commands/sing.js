...
module.exports.run = async function ({ api, event, args }) {
    ...
    try {
        ...
        // Tải ảnh thumbnail của bài hát đầu tiên
        const thumbnailUrl = data[0].thumbnail.url;
        const thumbnailPath = path.resolve(__dirname, 'cache', `thumb-${senderID}.jpg`);
        ...
        writer.on('finish', () => {
            api.sendMessage({
                body: `📝 Có ${link.length} kết quả trùng với từ khóa tìm kiếm của bạn:\n──────────────────\n${msg}\n\n📌 Reply (phản hồi) STT để tải nhạc`,
                attachment: createReadStream(thumbnailPath)
            }, threadID, (error, info) => {
                ...
                // Xóa ảnh sau khi gửi
                setTimeout(() => fs.existsSync(thumbnailPath) && fs.unlinkSync(thumbnailPath), 30000);
            }, messageID);
        });
