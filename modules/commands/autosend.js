        ...
        if (item.time === now) {
          ...
          if (item.attachment) {
            if (item.attachment === 'girl') {
              msg.attachment = global.khanhdayr.splice(0, 1);
            } else if (item.attachment === 'ani') {
              msg.attachment = global.vdanime.splice(0, 1);
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  switch (handleReply.type) {
    ...
    case "time":
      ...
      return api.sendMessage("Gửi ảnh/video đính kèm, hoặc nhập 'girl', 'ani', URL của ảnh/video (hoặc 's' nếu không cần):", threadID, (err, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
