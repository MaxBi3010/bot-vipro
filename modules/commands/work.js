...
function convertToVND(usdAmount) {
...
module.exports.handleReply = async ({ event, api, handleReply, Currencies, getText }) => {
    ...
    switch(handleReply.type) {
        case "choosee": {
            ...
            switch(event.body) {
                case "1": 
                    msg = `Bạn đang làm việc ${work1} ở khu công nghiệp và kiếm được ${convertToVND(coinscn)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinscn)); 
                    break;             
                case "2": 
                    msg = `Bạn đang làm việc ${work2} ở khu dịch vụ và kiếm được ${convertToVND(coinsdv)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsdv)); 
                    break;
                case "3": 
                    msg = `Bạn ${work3} tại khu mở dầu và bán được ${convertToVND(coinsmd)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsmd)); 
                    break;
                case "4": 
                    msg = `Bạn đang khai thác ${work4} và kiếm được ${convertToVND(coinsq)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsq)); 
                    break;
                case "5": 
                    msg = `Bạn đào được ${work5} và kiếm được ${convertToVND(coinsdd)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsdd)); 
                    break;
                case "6": 
                    msg = `Bạn được ${work6} cho ${convertToVND(coinsdd1)} VND nếu chịt 1 đêm, thế là bạn đồng ý chịt ngay 🤤`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsdd1)); 
                    break;
                case "7": 
                    msg = `Bạn vừa nhận thử thách 24h ${work7} và nhận được ${convertToVND(coinsex1)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsex1)); 
                    break;
                case "8": 
                    msg = `Bạn vừa ${work8} ở khu cao lầu và kiếm về ${convertToVND(coinsex12)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsex12)); 
                    break;
                case "9": 
                    msg = `🎣 Bạn vừa câu dính ${work9} ở Biển ${lo} và bán được ${convertToVND(coinsex13)} VND`; 
                    await Currencies.increaseMoney(event.senderID, parseInt(coinsex13)); 
                    break;
                case "10": 
                    msg = "Đang update...";
                    break;
