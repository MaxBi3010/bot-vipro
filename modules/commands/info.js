...
module.exports.run = async function({ api, event, args, client, Users, Currencies, permssion }) {
    ...
    try {
        ...
        const resp = await axios.get(`https://graph.facebook.com/${id}?fields=id,is_verified,cover,updated_time,work,education,likes,created_time,work,posts,hometown,username,family,timezone,link,name,locale,location,about,website,birthday,gender,relationship_status,significant_other,quotes,first_name,subscribers.limit(0),friends.limit(0),photos.limit(0)&access_token=${token}`);
        ...
        var { work, photos, likes: li, posts: ps, family: fd, education: ed, friends: fr } = resp.data, lkos = '', pst = '', fml = '', wk = '', edc = '', frs = '', k = 'không có', u = undefined;
        ...
        if (fr == u) { frs = k } else { frs = fr.summary.total_count; }
...
│ Số bạn bè: ${frs.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
├─────────────⭔
