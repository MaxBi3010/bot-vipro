...
const contact = () => api.shareContact(`⚠ Sai lệnh!\n🖲️ có phải lệnh  " ${checker.bestMatch.target} "\n📝dùng lệnh "menu" Để xem`, uid, threadID);
                const send = attachment => api.sendMessage({ body: `⚠️ Lệnh không tồn tại\n📝 ${threadSetting.PREFIX || PREFIX}menu để xem các lệnh hiện có\n✏️ Lệnh được thêm gần đây là: ${newFile}\n⏰ Uptime: ${H+$+M+$+S}`, attachment }, threadID, messageID);
                  let data;
                  if(adminbot.mode && adminbot.mode == "vdgai"){
                    if(global.khanhdayr.length == 0) return contact();
                    data = global.khanhdayr.splice(0,1);
                    if(data == null) return contact();
                    else return send(data);
                  }
                  else if(adminbot.mode && adminbot.mode == "vdcosplay"){
                    if(global.vdcosplay.length == 0) return contact();
                    data = global.vdcosplay.splice(0,1);
                    if(data == null) return contact();
                    else return send(data);
                  }
                   else if(adminbot.mode && adminbot.mode == "vdanime"){
                      if(global.vdanime.length == 0) return contact();
                      data = global.vdanime.splice(0,1);
                      if(data == null) return contact();
                      else return send(data);
                    }
                  else if(!adminbot.mode || !["vdgai","vdanime","vdcosplay"].includes(adminbot.mode)) return contact();
                    }
                  }
                });
              }
